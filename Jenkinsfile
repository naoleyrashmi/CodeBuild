pipeline {
        ...
        stage("Deploy App") {
            sh (script: '''
                ...
                aws cloudformation deploy --template-file /var/lib/jenkins/workspace/teghds/packaged.yaml --region us-east-2 --capabilities CAPABILITY_IAM CAPABILITY_AUTO_EXPAND --stack-name InitialSetup-${env.BUILD_NUMBER} --parameter-overrides PrimePartitionKey=${Prime-Partition-Key}  PrimePartitionKey1=${Prime-Partition-Key1} PrimePartitionKey2=${Prime-Partition-Key2} PrimePartitionKey3=${Prime-Partition-Key3}
